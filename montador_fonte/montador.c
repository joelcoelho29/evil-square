            ...
            switch (op_code)
            {
                ...
                */
                ...
                case STORE_CODE :
                 ...
                 /* ==============
                   Storee EndOrigem, EndDestino, Rx
                   ==============
                */
                
                case STOREE_CODE:
                    val2 = RecebeEndereco();            //end origem
                    parser_Match(',');
                    val3 = RecebeEndereco();            //end destino
                    parser_Match(',');
                    str_tmp1 = parser_GetItem_s();
                    val1 = BuscaRegistrador(str_tmp1);  //registrador auxiliar
                    free(str_tmp1);
                    str_tmp1 = ConverteRegistrador(val1);
                    str_tmp2 = NumPBinString(val2);         //end origem
                    str_tmp3 = NumPBinString(val3);         //end destino
                    sprintf(str_msg,"%s%s0000000",STOREE,str_tmp1); // STOREE = 110100 str_tmp1 = 000 1101000000000000
                    parser_Write_Inst(str_msg,end_cnt);
                    end_cnt += 1;
                    sprintf(str_msg,"%s",str_tmp2);
                    parser_Write_Inst(str_msg,end_cnt);
...
int BuscaInstrucao(char * nome)
{
    ...
    */
    ...
    else if (strcmp(str_tmp,STOREE_STR) == 0)
    {
        return STOREE_CODE;
    }
